...
private[deploy] class Master(
  ...
  override def receive: PartialFunction[Any, Unit] = {
    ...
    case RegisterApplication(description, driver) =>
      ...
      //  对当前master的状态进行判断，看是否是standby master
      //  如果是standby 不做任何处理
      if (state == RecoveryState.STANDBY) {
        // ignore, don't send response
      } else {
        logInfo("Registering app " + description.name)
        //  利用applicationdescription创建appcationInfo
        val app = createApplication(description, driver)
        //  注册appcation
        registerApplication(app)
        logInfo("Registered app " + description.name + " with ID " + app.id)
        //  使用持久化引擎，对appcationInfo进行持久化
        persistenceEngine.addApplication(app)
        //  反向向schedulerbackend发送消息
  ...
  private def registerApplication(app: ApplicationInfo): Unit = {
    //  拿到当前注册application的driver地址进行判断，是否是重复注册
    ...
    //  若已存在将当前application放入hashset缓存中进行去重处理
    apps += app
    //  将当前ApplicationInfo放入缓存，以及进行id、driver、address的一一绑定
    idToApp(app.id) = app
    endpointToApp(app.driver) = app
    addressToApp(appAddress) = app
    //  将当前ApplicationInfo加入等待调度队列waitingApps
    waitingApps += app
  }
